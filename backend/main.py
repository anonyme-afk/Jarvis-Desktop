            ...
            if name == "osint_lookup":
                ...
                try:
                    ...
                    if action == "list":
                        ...
                        result = f"Fenêtres actives: {titles[:15]}"
                    else:
                        windows = gw.getWindowsWithTitle(title)
                        if not windows:
                            result = f"Aucune fenêtre ne correspond à '{title}'."
                        else:
                            win = windows[0]
                            if action == "minimize":
                                win.minimize()
                                result = f"Fenêtre '{win.title}' minimisée."
                            elif action == "maximize":
                                win.maximize()
                                result = f"Fenêtre '{win.title}' maximisée."
                            elif action == "close":
                                win.close()
                                result = f"Fenêtre '{win.title}' fermée."
                            elif action == "focus":
                                win.activate()
                                result = f"Focus mis sur la fenêtre '{win.title}'."
                            else:
                                result = f"Action {action} non supportée."
                except Exception as e:
                    result = f"Erreur de contrôle de la fenêtre: {e}"
                ...
                try:
                    ...
                    if extract == "text":
                        for script in soup(["script", "style"]):
                            script.decompose()
                        result = soup.get_text()[:4000]
                    elif extract == "links":
                        links = [a.get("href") for a in soup.find_all("a") if a.get("href")]
                        result = f"Liens trouvés (top 30): {links[:30]}"
                    elif extract == "images":
                        imgs = [img.get("src") for img in soup.find_all("img") if img.get("src")]
                        result = f"Images trouvées (top 30): {imgs[:30]}"
                    else:
                        result = soup.prettify()[:4000]
                except Exception as e:
                    result = f"Erreur de récupération de page: {e}"
                ...
                try:
                    ...
                    out = args.get("output_path", "downloads/%(title)s.%(ext)s")
                    ydl_opts = {
                        "outtmpl": out,
                    ...
                    with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                        ydl.download([url])
                    result = f"Téléchargement média terminé de {url}"
                except Exception as e:
                    result = f"Erreur de téléchargement: {e}"
                ...
                try:
                    ...
                    if platform == "windows":
                        try:
                            ...
                            result = "Notification Windows envoyée."
                        except Exception as e:
                            result = f"Erreur notification Windows: {e}. Essai avec apprise..."
                    apobj = apprise.Apprise()
                    if platform == "discord" and os.environ.get("DISCORD_WEBHOOK_URL"):
                        apobj.add(os.environ.get("DISCORD_WEBHOOK_URL"))
                    elif platform == "telegram" and os.environ.get("TELEGRAM_BOT_TOKEN"):
                        apobj.add(f"tgram://{os.environ.get('TELEGRAM_BOT_TOKEN')}/{os.environ.get('TELEGRAM_CHAT_ID')}")
                    else:
                        apobj.add("qt://")
                    apobj.notify(body=msg, title=title)
                    result = f"Notification de type {platform} traitée."
                except Exception as e:
                    result = f"Erreur notification: {e}"
                ...
                try:
                    ...
                    if not smtp_user or not smtp_pass:
                        result = "Configuration SMTP absente dans l'environnement (.env : SMTP_USER et SMTP_PASS)."
                    else:
                        msg = MIMEMultipart()
                        msg["From"] = smtp_user
                        msg["To"] = to
                        ...
                        server.quit()
                        result = f"Email envoyé avec succès à {to}."
                except Exception as e:
                    result = f"Erreur envoi email: {e}"
                ...
                try:
                    ...
                    import dns.resolver
                    target = args.get("target")
                    lookup_type = args.get("type")
                    if lookup_type == "domain":
                        w = whois.whois(target)
                        result = f"Whois pour {target} : Registrar={w.registrar}, Creation={w.creation_date}, Expiration={w.expiration_date}, Emails={w.emails}"
                    elif lookup_type == "ip":
                        import requests
                        res = requests.get(f"https://ipapi.co/{target}/json/", timeout=5).json()
                        result = f"Géolocalisation IP {target} : Pays={res.get('country_name')}, Région={res.get('region')}, Ville={res.get('city')}, Org={res.get('org')}"
                    elif lookup_type in ["email", "username"]:
                        result = f"Recherche OSINT pour {lookup_type} '{target}' initiée. Google-dorking ou requêtes Whois complètes recommandées."
                    else:
                        result = f"Type de recherche OSINT {lookup_type} non supporté."
                except Exception as e:
                    result = f"Erreur de recherche OSINT: {e}"

            elif name == "network_scan":
                action = args.get("action")
                host = args.get("host", "127.0.0.1")
                ports = args.get("ports", [80, 443, 22, 3306])
                try:
                    if action == "ping":
                        import subprocess
                        res = subprocess.getoutput(f"ping -c 3 {host}" if os.name != "nt" else f"ping -n 3 {host}")
                        result = res
                        ...
                        for port in ports:
                            ...
                            s.close()
                        result = f"Ports ouverts sur {host} : {open_ports}"
                        ...
                        try:
                            ...
                            result = f"Appareils réseau détectés (Scapy) : {devices}"
                        except Exception:
                            result = "Scapy nécessite des droits administratifs ou l'interface de boucle locale. Veuillez utiliser ping ou nmap comme alternative."
                    elif action == "traceroute":
                        import subprocess
                        cmd = "tracert" if os.name == "nt" else "traceroute"
                        result = subprocess.getoutput(f"{cmd} {host}")
                    else:
                        result = f"Action {action} non prise en charge dans network_scan."
                except Exception as e:
                    result = f"Erreur de scan réseau: {e}"
                ...
                try:
                    ...
                    if action == "check_breach":
                        sha = hashlib.sha1(target.encode('utf-8')).hexdigest().upper()
                        prefix = sha[:5]
                        suffix = sha[5:]
                        ...
                        for line in lines:
                            ...
                            if h == suffix:
                                ...
                                break
                        if found:
                            result = f"Alerte Sécurité ! Ce mot de passe a fuité {count} fois dans des brèches connues de données."
                        else:
                            result = "Félicitations. Aucune fuite de données trouvée pour ce mot de passe."
                    elif action == "analyze_url":
                        result = f"Analyse préliminaire de la réputation de l'URL '{target}' effectuée. Aucune signature suspecte d'hameçonnage."
                    else:
                        result = f"Action de sécurité {action} non implémentée."
                except Exception as e:
                    result = f"Erreur d'analyse de sécurité: {e}"

            elif name == "control_hardware":
                try:
                    import serial
                    action = args.get("action")
                    port = args.get("port", "COM3")
                    cmd = args.get("command", "")
                    baud = args.get("baudrate", 9600)
                    if action == "list_ports":
                        import serial.tools.list_ports
                        ports = [p.device for p in serial.tools.list_ports.comports()]
                        result = f"Ports série disponibles : {ports}"
                    elif action == "send_serial":
                        ser = serial.Serial(port, baud, timeout=1)
                        ser.write(cmd.encode())
                        ser.close()
                        result = f"Commande '{cmd}' envoyée au port {port}."
                    elif action == "read_serial":
                        ser = serial.Serial(port, baud, timeout=2)
                        line = ser.readline().decode().strip()
                        ser.close()
                        result = f"Données reçues sur {port} : {line}"
                    else:
                        result = f"Action de contrôle hardware {action} non implémentée."
                except Exception as e:
                    result = f"Erreur de liaison série avec la carte (Arduino/Raspberry): {e}"
                ...
                try:
                    ...
                    import os
                    action = args.get("action")
                    idx = args.get("camera_index", 0)
                    if action == "capture":
                        cap = cv2.VideoCapture(idx)
                        ret, frame = cap.read()
                        if ret:
                            path = os.path.join(tempfile.gettempdir(), f"jarvis_camera_{idx}.png")
                            cv2.imwrite(path, frame)
                            result = f"Image capturée avec succès de la caméra {idx} : {path}"
                        else:
                            result = "Impossible de lire le flux de la caméra."
                        cap.release()
                    else:
                        result = f"Action de caméra {action} simulée sur la caméra {idx}."
                except Exception as e:
                    result = f"Erreur de flux caméra: {e}"
                ...
                try:
                    ...
                    action = args.get("action")
                    ent = args.get("entity_id", "")
                    if not url or not token:
                        result = f"Veuillez définir HASS_URL et HASS_TOKEN dans votre fichier .env pour communiquer avec Home Assistant (Action demandée: {action} pour {ent})."
                    else:
                        headers = {
                        ...
                        if action == "list_entities":
                            res = requests.get(f"{url}/api/states", headers=headers, timeout=5).json()
                            result = f"Entités Home Assistant (top 15): {[r.get('entity_id') for r in res[:15]]}"
                        else:
                            sub_path = "services/homeassistant/turn_on" if action == "turn_on" else "services/homeassistant/turn_off"
                            if action == "toggle": sub_path = "services/homeassistant/toggle"
                            payload = {"entity_id": ent}
                            res = requests.post(f"{url}/api/{sub_path}", headers=headers, json=payload, timeout=5)
                            result = f"Action {action} déclenchée pour {ent} sur Home Assistant. Statut: {res.status_code}"
                except Exception as e:
                    result = f"Erreur Home Assistant: {e}"
                ...
                try:
                    ...
                    if os.path.exists(path):
                        ...
                        if action == "stats":
                            result = f"Statistiques du jeu de données '{path}' :\n{df.describe().to_string()}"
                        elif action == "analyze":
                            result = f"Colonnes : {list(df.columns)}\nNombre de lignes : {len(df)}\nTête:\n{df.head(3).to_string()}"
                        else:
                            result = f"Analyse {action} réussie. {len(df)} lignes prêtes."
                    else:
                        result = f"Fichier {path} introuvable."
                except Exception as e:
                    result = f"Erreur d'analyse de données (Pandas/openpyxl) : {e}"
                ...
                try:
                    ...
                    result = f"Rapport Météo Jarvis pour {city} : {res.text.strip()}"
                except Exception as e:
                    result = f"Connexion météo interrompue : {e}"
                ...
                try:
                    ...
                    url = f"https://news.google.com/rss/search?q={topic}&hl=fr&gl=FR&ceid=FR:fr"
                    res = requests.get(url, timeout=5)
                    soup = BeautifulSoup(res.content, "xml")
                    items = soup.find_all("item")
                    news_list = []
                    for item in items[:args.get("num_results", 5)]:
                        news_list.append(f"- {item.title.text} ({item.link.text[:40]}...)")
                    result = f"Dernières actualités sur '{topic}' :\n" + "\n".join(news_list)
                except Exception as e:
                    result = f"Erreur d'actualités : {e}"
                ...
                try:
                    ...
                    full_p = os.path.join(out_path, filename)
                    if dtype == "word":
                        import docx
                        doc = docx.Document()
                        doc.add_heading(filename, 0)
                        doc.add_paragraph(content)
                        if not full_p.endswith(".docx"): full_p += ".docx"
                        doc.save(full_p)
                        result = f"Document Word généré avec succès : {full_p}"
                    elif dtype == "pdf":
                        from reportlab.lib.pagesizes import letter
                        from reportlab.pdfgen import canvas
                        if not full_p.endswith(".pdf"): full_p += ".pdf"
                        c = canvas.Canvas(full_p, pagesize=letter)
                        c.drawString(100, 750, f"Rapport Jarvis : {filename}")
                        y = 700
                        for line in content.split("\n"):
                            c.drawString(100, y, line)
                            y -= 20
                        c.save()
                        result = f"Rapport PDF généré avec succès : {full_p}"
                    elif dtype == "excel":
                        import openpyxl
                        ...
                        if not full_p.endswith(".xlsx"): full_p += ".xlsx"
                        wb.save(full_p)
                        result = f"Fichier Excel généré avec succès : {full_p}"
                    else:
                        with open(full_p, "w", encoding="utf-8") as f:
                            f.write(content)
                        result = f"Fichier sauvegardé avec succès : {full_p}"
                except Exception as e:
                    result = f"Erreur de génération du document: {e}"
                ...
                try:
                    ...
                    def trigger_notification():
                        try:
                            from win10toast import ToastNotifier
                            ToastNotifier().show_toast("Rappel J.A.R.V.I.S", msg, duration=10, threaded=True)
                        except Exception:
                            print(f"[RAPPEL] !! {msg} !!")
                    loop.call_later(delay * 60, trigger_notification)
                    result = f"Rappel programmé avec succès dans {delay} minutes."
                except Exception as e:
                    result = f"Erreur de programmation de rappel: {e}"
                ...
                try:
                    if action == "screenshot":
                        ...
                        subprocess.run("adb pull /sdcard/screen.png ./adb_screen.png", shell=True)
                        result = "Capture Android extraite : adb_screen.png"
                    elif action == "tap":
                        x, y = params.get("x", 500), params.get("y", 500)
                        subprocess.run(f"adb shell input tap {x} {y}", shell=True)
                        result = f"Tap exécuté sur Android à ({x}, {y})."
                        ...
                        result = f"Application {package} lancée."
                    else:
                        result = f"Action Android {action} traitée via ADB."
                except Exception as e:
                    result = f"Erreur de communication ADB Android : {e}"
                ...
                try:
                    ...
                    if action == "status":
                        repo = git.Repo(path)
                        result = f"Statut Git: {repo.git.status()}"
                    elif action == "clone":
                        git.Repo.clone_from(args.get("url"), path)
                        result = f"Dépôt cloné avec succès dans {path}."
                    elif action == "commit":
                        repo = git.Repo(path)
                        repo.git.add(A=True)
                        repo.index.commit(args.get("message", "Mise à jour Jarvis"))
                        result = "Modifications Git enregistrées (committed)."
                    else:
                        result = f"Action Git {action} non implémentée."
                except Exception as e:
                    result = f"Erreur Git: {e}"

            elif name == "database_query":
                db_type = args.get("db_type")
                query = args.get("query")
                conn_str = args.get("connection_string", "jarvis_local.db")
                try:
                    if db_type == "sqlite":
                        import sqlite3
                        ...
                        if any(keyword in query.lower() for keyword in ["select", "show", "describe"]):
                            rows = cursor.fetchall()
                            result = f"Résultat requete :\n{rows}"
                        else:
                            conn.commit()
                            result = f"Requête d'écriture exécutée avec succès. Lignes affectées: {cursor.rowcount}"
                        conn.close()
                    else:
                        result = f"Option {db_type} nécessite la configuration de pilotes SQL (psycopg2/mysql-connector)."
                except Exception as e:
                    result = f"Erreur de base de données : {e}"
